...
class Disk:
    def __init__(self, addr, addrDesc, lateAddr, lateAddrDesc,
        ...
        if self.graphics:
            ...
            self.canvas = Canvas(self.root, width=510, height=560 + ((tmpLen / 20) * 20))
            self.canvas.pack()
        ...
        self.tracks[3] = self.tracks[2] - self.trackWidth
...
parser.add_option('-z', '--zoning',          default='10,15,20,25,30',  help='Angles between blocks on outer,middle,inner tracks',      action='store', type='string', dest='zoning')
